...
define(['jquery', 'app/util', 'app/selectors', 'app/bib'], function ($, util, selectors, bib) {
    ...
    function createTag(tag, options, frequency, activeTags, tagFrequencySelector) {
        ...
        $('<span>', {
            class: 'text',
            html: util.latexToHtml(getTagDisplayName(tag))
        }).appendTo(tagDiv);
        ...
        tagDiv.mouseover(function () {
            if (!tagDiv.hasClass('tooltipstered')) {
                var tooltipDiv = $('<div>');
                $('<h3><span class="label">' + options.field + ': </span>' + util.latexToHtml(getTagDisplayName(tag)) + '</h3>').appendTo(tooltipDiv);
    ...
    function appendTagDivs(name, title, tagDivs, element) {
        ...
        if (name) {
            var displayName = bib.tagCategories[name] && bib.tagCategories[name]['displayName'] ? bib.tagCategories[name]['displayName'] : name;
            var labelDiv = $('<span>', {
                class: 'label tooltip',
                title: title,
                text: displayName + ": "
    ...
    function getTagDisplayName(tag) {
        if (bib.authorizedTags[tag] && bib.authorizedTags[tag]['displayName']) {
            return bib.authorizedTags[tag]['displayName'];
        }
